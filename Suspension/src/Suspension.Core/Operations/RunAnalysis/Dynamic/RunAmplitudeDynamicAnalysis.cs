        ...
        where TRunDynamicAnalysisRequest : RunGenericDynamicAnalysisRequest
    {
        ...
        protected async override Task<RunAmplitudeDynamicAnalysisResponse> ProcessOperationAsync(TRunAmplitudeDynamicAnalysisRequest request)
        {
                ...
                using (StreamWriter streamWriter = new(solutionFullFileName))
                {
                    ...
                    foreach (TRunDynamicAnalysisRequest runDynamicAnalysisRequest in runDynamicAnalysisRequestList)
                    {
                        ...
                        else
                        {
                            ...
                            string fileExtension = Path.GetExtension(runDynamicAnalysisResponse.Data.FullFileName);
                            string requestFullFileName = runDynamicAnalysisResponse.Data.FullFileName.Replace(fileExtension, ".json");
                            ...
                            response.Data.FullFileNames.Add(runDynamicAnalysisResponse.Data.FullFileName);
            ...
            finally
            {
                ...
                //    Created (202) - If the number of full file names in response divided by 2 is equal to the number of requests to process.
                //    Partial Content (206) - Otherwise.
                // OBS.: It has a "divided by 2" because for each request it must have two files: one with the all request content and one
                //       with the results.
                if (response.Data.FullFileNames.Count / 2 == runDynamicAnalysisRequestList.Count)
                {
